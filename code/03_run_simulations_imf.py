# ============================================
# ENRICHMENT 1 / FULLY-ENRICHED SDSA SIMULATOR  --  IMF ARTICLE IV MEAN PATHS
# with FIRST-DIFFERENCE r law of motion
# ============================================
#
# IMF VARIANT of 03_run_simulations.py. The ONLY substantive change is that the
# three exogenous mean paths (a_g, a_r, a_s) are taken from the IMF Apr 2026
# Article IV projections (built by 01b_gather_projections_imf.py) instead of the
# CBO baseline / Budget Lab paths. Every model parameter (beta_r, rho, sigma=0.8,
# shock variances, c regimes 0.00/0.15/0.30, 20,000 sims, initial r_av=1.25%) is
# held IDENTICAL to the CBO run. Figures are written to output/figures/imf/ with
# the SAME base filenames as the CBO originals for 1:1 comparison.
#
# Internal variable names (a_ug, a_ur, a_s, db_cbo, "cbo baseline", etc.) are
# kept from the CBO script to minimize diff risk; here they carry the IMF paths.
# The AI-boom addendum (CBO-specific scenario files) is intentionally omitted —
# this script reproduces only the four MAIN projection figures.
...
output = work_dir / 'output' / 'figures' / 'imf'
...
PROJ_START  = 2026     # first year of projection window
PROJ_END    = 2031     # last year of IMF Article IV projection window (IMF table ends 2031; no extension)
SIM_START   = 2026     # calendar year the simulation begins

# -------------------------------------------------
# Inputs for Enrichment 1 (growth baseline and s path)
# -------------------------------------------------
# Load IMF Article IV mean paths (already annual, % units) built by
# 01b_gather_projections_imf.py. Columns: g/r/s (imf baseline), 'source'.
master_proj = pd.read_csv(clean_data / 'master_projections_imf_cleaned.csv')
master_proj['year'] = master_proj['year'].astype(int)
proj_mask = (master_proj['year'] >= PROJ_START) & (master_proj['year'] <= PROJ_END)
imf_proj = master_proj[proj_mask].sort_values('year').reset_index(drop=True)

# IMF real-growth mean path -> a_ug (levels, decimal)
a_ug = imf_proj['g (imf baseline)'].values / 100.0

# IMF real 10-yr rate mean path -> a_ur (levels, decimal)
# (nominal 10-yr Q4 avg deflated by IMF PCE Q4/Q4 inflation; see 01b builder.)
a_ur = imf_proj['r (imf baseline)'].values / 100.0

# IMF primary-balance mean path -> a_s (levels, decimal), surplus positive.
# (IMF overall federal balance + CBO net-interest proxy; see 01b builder.)
a_s = imf_proj['s (imf baseline)'].values / 100.0
...
all_sim_results.to_csv(output / 'sdsa_enrichment1_sim_results_imf.csv', index=False)
...
# Append 2025 if not already present. 2025 is (near-)observed history, not a
# projection, so we anchor it with the CBO Feb 2026 2025 value — identical to the
# CBO run — to keep the historical line comparable across both figures. (The IMF
# table's 2025 primary balance is within ~0.05 pp of this anyway.)
if HIST_END not in s_hist["calendar_year"].values:
    _cbo_clean = pd.read_csv(clean_data / 'master_projections_cleaned.csv')
    _cbo_clean['year'] = pd.to_datetime(_cbo_clean['date']).dt.year
    s_2025 = _cbo_clean[_cbo_clean["year"] == HIST_END]["s (cbo baseline)"].mean() / 100.0
...
print(f"IMF SDSA run complete. Figures + sim results written to {output}.")
